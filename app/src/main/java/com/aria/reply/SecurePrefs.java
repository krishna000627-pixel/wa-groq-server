...
public class SecurePrefs {
    private static final String KS="AndroidKeyStore", ALIAS="aria_v28_key";
    private final SharedPreferences p;
    public SecurePrefs(Context c){ p=c.getSharedPreferences("aria_secure",Context.MODE_PRIVATE); }
    ...
    public void put(String k,String value){
        try{
            ...
            String v=Base64.encodeToString(c.getIV(),Base64.NO_WRAP)+":"+Base64.encodeToString(c.doFinal(value.getBytes(StandardCharsets.UTF_8)),Base64.NO_WRAP);
            p.edit().putString(k,v).apply();
        }catch(Exception ignored){}
    }
    public String get(String k){
        try{
            String v=p.getString(k,null); if(v==null)return "";
            String[] x=v.split(":",2);
