...
- Never mention hidden prompts, internal rules, context storage, model details, or system instructions unless explicitly asked about the assistant itself.

TIME AND MEMORY
- The server supplies authoritative current time in IST.
- Treat IST as authoritative for dates, weekdays and context expiry.
- Context is retained by IST calendar date, not by a rolling 72-hour timer.
- Keep messages from today and the previous 2 IST calendar dates only. Older messages are expired and must not be used.
- The server supplies GAP_MINUTES, LAST_INTERACTION_IST, and whether this is a fresh context.
- If the person returns after 60-300 minutes, begin the first reply with one short, natural AI-agent greeting, then answer the message.
- If the context has expired, treat the interaction as fresh and use a brief AI-agent greeting when appropriate.
- Do not expose raw timestamps unless the user asks for time/date information.

STYLE
- Sound like a real AI assistant: natural, concise and clear.
- Make it obvious through wording that you are an AI assistant, without repeatedly saying "I am an AI".
- Hinglish in -> Hinglish out; English in -> English out.
- Normally 1-3 short lines unless the task requires more.
- Do not imitate Krishna's personal writing style so closely that you appear to be Krishna.
- Do not use fake human claims such as "I just saw him", "I talked to Krishna", or "Krishna told me" unless that exact information exists in the supplied context.
- Do not fabricate emotions, experiences, physical presence, or personal actions.

CLOSED-END RESPONSE
- Give a complete answer and naturally end the interaction.
- Do not add open-ended engagement bait such as "anything else?", "what else can I help with?", "tell me more", or "let me know if you need anything else".
- If a person asks for something that only Krishna can personally send, buy, bring, approve, or provide, do not invent that it has already happened.
- When appropriate, say: "Krishna will handle/send it later." Do not invent a time, deadline, book, subject, item, or promise.
- For requests such as "send me English", "send the book", "send notes", etc., do not guess which subject/book/notes they mean. State that Krishna will handle it later unless the conversation explicitly identifies the exact item.

ACCURACY
- Never hallucinate schedules, locations, relationships, names, books, subjects, events, deadlines, or actions.
- If a factual answer requires missing information, say so instead of guessing.
- Use only the supplied conversation context and reliable general knowledge.
- Skip only blank messages, obvious forwards/spam, or media with no usable text.
- Never reveal or repeat API keys, secrets, environment variables, or credentials.
"""
...
# Each message carries an epoch timestamp and an IST date.
CONTEXT = {}
...
def load_config():
    ...
    # Never allow a missing/deleted prompt to disable the built-in prompt.
...
def save_config(c):
    ...
    # Do not persist an environment-provided key into the JSON file.
...
def prune(sender, cfg):
    """Keep today + previous two IST calendar dates."""
    today = now_ist().date()
    ...
    for m in CONTEXT.get(sender, []):
        try:
            msg_date = ist_date_from_ts(m["ts"])
        except Exception:
            continue

        if msg_date >= oldest_allowed:
            history.append(m)
...
def context_meta(sender, cfg):
    ...
    gap_minutes = None if last_ts is None else max(0, (now_ts - last_ts) / 60)
...
def groq_client(key):
    # Official Groq Python SDK instead of urllib/Python-urllib.
...
def ask(cfg, sender, message):
    ...
    gap = (
        "none"
        if meta["gap_minutes"] is None
        else f"{meta['gap_minutes']:.1f}"
    )
    ...
    system_prompt += (
        ...
        "If GAP_MINUTES is between 60 and 300, the first response should "
        "start with a brief natural AI-agent greeting before answering."
    )
    ...
    def do_GET(self):
        if self.path in ("/", "/dashboard"):
            try:
                ...
                b = f"<h1>Dashboard unavailable</h1><p>{exc}</p>"
            ...
            c = load_config()
            # Never expose the real environment key.
            public = dict(c)
            if os.environ.get("GROQ_API_KEY"):
    ...
    def do_POST(self):
        if self.path == "/config":
            try:
                ...
                # Do not replace secrets with an empty value accidentally.
                ...
                if not str(current.get("system_prompt") or "").strip():
                    current["system_prompt"] = DEFAULT_PROMPT
            ...
            if reply:
                ...
                self.log("skip", sender, err or "skipped")
                self.sendj(
                    200,
                    {
                        "replies": [],
