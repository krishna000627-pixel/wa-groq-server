...
class MainActivity : Activity() {
    ...
    private fun panel(color:Int, radius:Int=24):GradientDrawable=GradientDrawable().apply{setColor(color);cornerRadius=dp(radius).toFloat()}
    private fun clay(title:String,value:String,color:Int,icon:Int?=null,action:(()->Unit)?=null):LinearLayout=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(17),dp(15),dp(17),dp(15));background=panel(color);elevation=dp(5).toFloat();layoutParams=LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,dp(6),0,dp(6))};addView(LinearLayout(this@MainActivity).apply{gravity=Gravity.CENTER_VERTICAL;if(icon!=null)addView(ImageView(this@MainActivity).apply{setImageResource(icon);layoutParams=LinearLayout.LayoutParams(dp(28),dp(28)).apply{setMargins(0,0,dp(10),0)}});addView(tv(title,12f,muted).apply{typeface=Typeface.DEFAULT_BOLD},LinearLayout.LayoutParams(0,-2,1f))});addView(tv(value,16f,ink).apply{setPadding(0,dp(8),0,0)});if(action!=null){setOnClickListener{action()}}}
    private fun button(label:String, icon:Int, color:Int, action:()->Unit):LinearLayout=LinearLayout(this).apply{gravity=Gravity.CENTER_VERTICAL;setPadding(dp(16),dp(12),dp(16),dp(12));background=panel(color,20);elevation=dp(4).toFloat();layoutParams=LinearLayout.LayoutParams(-1,dp(58)).apply{setMargins(0,dp(6),0,dp(6))};addView(ImageView(this@MainActivity).apply{setImageResource(icon);layoutParams=LinearLayout.LayoutParams(dp(24),dp(24)).apply{setMargins(0,0,dp(12),0)}});addView(tv(label,15f,ink));setOnClickListener{action()}}
    ...
    private fun home(){
        ...
        steps.forEach{(name,ok)->pipe.addView(tv("${if(ok)"●" else "○"}  $name",14f,if(ok) green else muted).apply{setPadding(dp(6),dp(7),0,dp(7))})};pipe.background=panel(surface,24);pipe.elevation=dp(4).toFloat();l.addView(pipe)
