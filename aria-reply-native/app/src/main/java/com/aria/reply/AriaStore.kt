...
class AriaStore(context: Context) {
    ...
    fun hasApiKey(): Boolean = prefs.contains("api_blob") && apiKey().isNotBlank()
    ...
    fun clearApiKey() {
        prefs.edit().remove("api_iv").remove("api_blob").apply()
    }
    ...
    fun logEvent(message: String, success: Boolean? = null) {
        ...
        val next = (existing + item).takeLast(80).toSet()
        prefs.edit().putStringSet("events", next).apply()
    }
